...
def extract_excel_specific_days_setting(excel_path):
    ...
    try:
        if not excel_path or not os.path.exists(excel_path):
            return "特定末尾3の日, 5の付く日, 6の付く日, 月日ゾロ目"
...
def run_gemini_analysis(api_key, context, target_date, excel_file=None):
    if excel_file and os.path.exists(excel_file):
        ...
        specific_days_rule = "特定末尾3の日、5の付く日、6の付く日・月日ゾロ目"
...
【分析対象店舗の特性】
店舗名：エランドール
- 分析対象店舗（エランドール）は毎日営業データをアナスロにて公開するデイリーホールです。
- 店舗の特定イベント日ルールは【特定末尾3の日、5の付く日、6の付く日・月日ゾロ目】です（Excel⚙️特定日末尾設定：{specific_days_rule}）。
...
<body class="min-h-screen p-4 md:p-8">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            ...
            if (dates.length > 0) {{
                const latestDate = dates[0]; // Index 0 is the NEWEST date!
        ...
        function calculateOverallAIAccuracy(targetDate) {{
            ...
            statElem.textContent = accuracy + '%';
            
            const detailElem = document.getElementById('stat-ai-accuracy-detail');
            if (detailElem) {{
                detailElem.textContent = '【' + (targetDate || '最新') + '迄の累計】 ' + correct + '勝 / ' + evaluated.length + '台';
            }}
        }}
...
def main():
    ...
    if not store_name:
        store_name = input("Enter Store Name (e.g. エランドール): ")
